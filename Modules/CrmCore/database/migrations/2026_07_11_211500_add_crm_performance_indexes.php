<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * @var array<int, array{table: string, name: string, columns: array<int, string>}>
     */
    private array $indexes = [
        [
            'table' => 'crm_reservations',
            'name' => 'crm_reservations_user_start_idx',
            'columns' => ['user_id', 'start_at'],
        ],
        [
            'table' => 'crm_reservations',
            'name' => 'crm_reservations_status_start_idx',
            'columns' => ['status', 'start_at'],
        ],
        [
            'table' => 'crm_equipment_rentals',
            'name' => 'crm_equipment_rentals_user_start_idx',
            'columns' => ['user_id', 'start_at'],
        ],
        [
            'table' => 'crm_equipment_items',
            'name' => 'crm_equipment_items_active_site_sort_idx',
            'columns' => ['active', 'site_id', 'sort_order', 'name'],
        ],
        [
            'table' => 'crm_equipment_categories',
            'name' => 'crm_equipment_categories_active_sort_idx',
            'columns' => ['active', 'sort_order', 'name'],
        ],
        [
            'table' => 'crm_vehicles',
            'name' => 'crm_vehicles_active_site_name_idx',
            'columns' => ['active', 'site_id', 'name'],
        ],
        [
            'table' => 'crm_leave_entries',
            'name' => 'crm_leave_entries_period_status_idx',
            'columns' => ['start_date', 'end_date', 'status'],
        ],
        [
            'table' => 'crm_leave_employees',
            'name' => 'crm_leave_employees_active_sort_name_idx',
            'columns' => ['active', 'sort_order', 'name'],
        ],
    ];

    public function up(): void
    {
        foreach ($this->indexes as $index) {
            $this->addIndex($index['table'], $index['name'], $index['columns']);
        }
    }

    public function down(): void
    {
        foreach (array_reverse($this->indexes) as $index) {
            $this->dropIndex($index['table'], $index['name']);
        }
    }

    /**
     * @param  array<int, string>  $columns
     */
    private function addIndex(string $table, string $name, array $columns): void
    {
        if (! $this->canUseColumns($table, $columns) || $this->hasIndex($table, $name)) {
            return;
        }

        Schema::table($table, function (Blueprint $blueprint) use ($columns, $name): void {
            $blueprint->index($columns, $name);
        });
    }

    private function dropIndex(string $table, string $name): void
    {
        if (! Schema::hasTable($table) || ! $this->hasIndex($table, $name)) {
            return;
        }

        Schema::table($table, function (Blueprint $blueprint) use ($name): void {
            $blueprint->dropIndex($name);
        });
    }

    /**
     * @param  array<int, string>  $columns
     */
    private function canUseColumns(string $table, array $columns): bool
    {
        if (! Schema::hasTable($table)) {
            return false;
        }

        foreach ($columns as $column) {
            if (! Schema::hasColumn($table, $column)) {
                return false;
            }
        }

        return true;
    }

    private function hasIndex(string $table, string $name): bool
    {
        return collect(Schema::getIndexes($table))
            ->contains(fn (array $index): bool => strcasecmp((string) ($index['name'] ?? ''), $name) === 0);
    }
};
